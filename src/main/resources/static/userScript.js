...
function loadUserTable() {
    $.getJSON("/front/api/users/list", function (users) {
...
function addNewUserRequest() {
    const myScript = async function (event) {
        ...
        let response = await fetch('/front/api/users', {
            method: 'POST',
            headers: {
...
function showModalForEditUser(userId) {
    ...
    $.getJSON("/front/api/users/" + userId, function (user) {
...
async function deleteUserRequest(userId) {
    if (confirm('Are you sure you want to delete this user?') === true) {
        let response = await fetch('/front/api/users/' + userId, {
            method: 'DELETE',
        });
        let result = await response.json();
        refreshTableData();
